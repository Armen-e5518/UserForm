...
function SetHtmlData(data) {
    ...
    $.each(index, function (index, value) {
        ...
        if (form_name.indexOf('textarea') >= 0
            ...
            if (form_value) {
                var href = location.protocol + "//" + document.domain + '/admin/forms/download?file=' + form_value;
...
function SavePdfContent() {
    ...
    $('#pdf-content input').each(function () {
        if (!$(this).hasClass('text-box-table')) {
            ...
            if (type == 'text' || type == 'email' || type == 'number' || type == 'tel') {
                $(this).after('<br><div style="width: 100%; border: 1px solid black; padding: 1px 6px;"><span  style="display: block; font-style: italic;  font-weight: bold;">' + $(this).val() + '</span> </div>');
                $(this).remove();
            }
            if (type == 'radio') {
                if ($(this).is(':checked')) {
            ...
            if (type == 'checkbox') {
                if ($(this).is(':checked')) {
                    $(this).after('<span><img style="margin-right: 3px; margin-top: 2px" src="/admin/images/form-img/active-che.png" width="15px" alt=""></span>');
                    $(this).remove();
                } else {
                    $(this).after('<span><img style="margin-right: 3px; margin-top: 2px"src="/admin/images/form-img/empty-che.png" width="15px" alt=""></span>');
                    $(this).remove();
                }
            }
        } else {
            var type = $(this).attr('type');
            if (type == 'text' || type == 'email' || type == 'number' || type == 'tel') {
                $(this).after('<span  style="display: block; font-style: italic;  font-weight: bold;">' + $(this).val() + '</span>');
                $(this).remove();
            }
        }
    })
    ...
    $('#pdf-content select').each(function () {
        $(this).after('<div style="width: 100%; border: 1px solid black;"><span style="display: block;  font-weight: bold; font-style: italic;" >' + $(this).val() + '</span></div>');
        $(this).remove();
    })
    ...
    $('#pdf-content textarea').each(function () {
        $(this).after('<div style="width: 100%; padding: 1px 6px; border: 1px solid black;"><span style="font-weight: bold;  font-style: italic;">' + $(this).val() + '</span></div>');
        $(this).remove();
    })
